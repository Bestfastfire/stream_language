    ...
    return StreamBuilder(
      ...
      builder: (context, snapshot){
        ...
        return builder(
          snapshot.data,
          screenRoute,
          languageBloc.defaultRoute != null ? snapshot.data[languageBloc.defaultRoute] : []
        );
      },
    );
  }
...
class LanguageBloc implements BlocBase{
  ...
  final BehaviorSubject<Map<dynamic, dynamic>> _languageDb = BehaviorSubject<Map<dynamic, dynamic>>();
  final BehaviorSubject<Map<dynamic, dynamic>> _languageList = BehaviorSubject<Map<dynamic, dynamic>>();
  ...
  LanguageBloc._internal({this.child, this.defaultLanguage, this.lastPrefix, this.defaultRoute});

  Future<void> init() async{
    this.lastPrefix ??= defaultLanguage;
    _languageDb.sink.add(
        (await FirebaseDatabase.instance.reference().child(child).once()).value
    );

    FirebaseDatabase.instance.reference().child(child)
        .onValue.listen((v) => _languageDb.sink.add(v.snapshot.value));
  ...
  Future<void> changeLanguage(String prefix) async{
    if(_languageDb.value[prefix] == null){
      ...
      print('setting prefix with defaultLanguage because prefix: $prefix dont exists in database!');
    }
    ...
    if(this.lastPrefix != (prefix)){
      ...
      print('language dont changed because informed prefix is the same as current prefix: $prefix');
    }
  }
  ...
  Future<dynamic> showAlertChangeLanguage({@required context, @required String title, @required String btnNegative}) async{
        ...
        content: Container(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              return Material(
                color: currentValue['config']['prefix'] == out[index]['prefix'] ? Colors.blueAccent[700] : Colors.transparent,
                child: ListTile(
                  leading: CountryPickerUtils.getDefaultFlagImage(
                      Country(
                          isoCode: out[index]['iso-code']
                      )
                  ),
                  selected: currentValue['config']['prefix'] == out[index]['prefix'],
                  title: Text(
                    out[index]['title'].toString(),
                    style: TextStyle(
                        color: currentValue['config']['prefix'] == out[index]['prefix'] ? Colors.white : Colors.black
                    ),
                  ),
                  onTap: (){
...
class FirstLanguageStart extends StatelessWidget {
  ...
  FirstLanguageStart({@required this.future, @required this.builder, this.loadWidget});
